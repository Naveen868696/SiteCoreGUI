...
public class UnHappyTests3 extends base {
	...
	public void Links() throws IOException, InterruptedException
	{
		...
		driver.switchTo().frame(driver.findElement(By.xpath("//iframe[@name = 'intercom-tour-frame']"))); // Navigate to new Frame
		dp.getWidgetFrameCancel().click(); // Click on Cancel
		dp.getlinktoAddWidget().click(); // Click on Link Create Widget Agian
		driver.switchTo().frame(driver.findElement(By.xpath("//iframe[@name = 'intercom-tour-frame']"))); // Navigate to new Frame
		dp.getWidgetFrameCancel().click(); //Click on cancel
	}
